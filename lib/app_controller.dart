...
class AppController {
  ...
  CardModel getSoulCard() {
    ...
    return soulCardList.where((c) => (sum - 1) % 9 + 1 == c.number).first;
  }

  void goUrl(String url) => launchUrl(Uri.parse(url));
  ...
  Future<void> back(BuildContext context, Widget page) => Navigator.push(
    context,
    PageRouteBuilder(
      transitionsBuilder: (context, animation, secondaryAnimation, child) =>
